<!--Test Oracle file for UBC CPSC304 2018 Winter Term 1
  ...
  <html>
    ...
    <body>
        <nav class="nav">
            <a href="new-deliverable.php" class="site-name">ZooKeeper</a> 
        </nav>
        <div class="container">
        <h2>Reset</h2>
        ...
        <form method="POST" action="new-deliverable.php"> <!--refresh page when submitted-->
            ...
            ConservationStatus: <input type="text" name="ConservationStatus"> <br /><br />
        ...
        Select Table: <select name="fromParam">
                <option value="Zoo1" >Zoo1</option>
                <option value="Zoo2" >Zoo2</option>
                <option value="ZooManager" >Zoo Manager</option>
                <option value="Exhibits" >Exhibits</option>
                <option value="Habitat1" >Habitat1</option>
                <option value="Habitat2" >Habitat2</option>
                <option value="FeatureAnimal1" >FeatureAnimal1</option>
                <option value="ResidentAnimal1" >Resident Animal1</option>
                <option value="FoodVendor1" >FoodVendor1</option>
                <option value="FoodVendor2" >FoodVendor1</option>
                <option value="Animal1" >Animal1</option>
                <option value="Order1" >Order1</option>
                <option value="Ticket1" >Ticket1</option>
                <option value="Ticket2" >Ticket2</option>
                <option value="TemporaryVisit" >TemporaryVisit</option>

            </select>
            <input type="hidden" id="selectionRequest" name="selectionRequest">
            Column Name(s): <input type="text" name="selectParam"> <br /><br />
            <!-- FROM: <input type="text" name="fromParam"> <br /><br /> -->
            Condition (var1): <input type="text" name="whereParam1"> <br /><br />
            <input type="submit" name="selectSubmit" class="button"></p>
        </form>
        ...
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
            ...
            Table Name(s): <input type="text" name="selectAtt"> <br /><br />
            Column Name(s): <input type="text" name="fromTab"> <br /><br />
            <input type="submit" name="projectionSubmit" class="button"></p>
        </form>
        ...
        <p>The values are case sensitive and if you enter in the wrong case, the update statement will not do anything also there would be an error on updating animalID/habitatID due to integrity constraints.</p>
        ...
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
            ...
            Condition: <input type="text" name="whereParam"> <br /><br />
            <input type="submit" name="joinAnimals" class="button"></p>
        </form>

        <hr />


        <h2>Food Vendors HAVING Rating Higher than AVERAGE Rating of FoodVendors</h2>
        <p>FINDS VendorName and FoodType Having higher rating than Average FoodVendors </p>
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
            <input type="hidden" id="havingRequest" name="havingRequest">
            <input type="submit" name="having" class="button"></p>
        </form>

        <hr />


        <h2>Prominent Species in Popular Exhibits </h2>
        <p>FINDS habitatType and their average vistor capacity where popularityRating is higher than 6. This query is groupped by habitatType</p>
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
            <input type="hidden" id="groupRequest" name="groupRequest">
            <input type="submit" name="group" class="button"></p>
        </form>

        <hr />

        <h2> Average Age of FeaturedAnimal whose PopularityRating is higher than Average PopularityRating GROUPPED BY SPECIES</h2>
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
            <input type="hidden" id="nestgroupRequest" name="nestgroupRequest">
            <input type="submit" name="nestgroup"></p>
        </form>

        <hr />


        <h2> Species of Resident Animals which are presented in Exhibits</h2>
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
        ...
        <h2>View the Feature Animals</h2>
        <form method="GET" action="new-deliverable.php"> <!--refresh page when submitted-->
        ...
        function handleUpdateRequest() {
            ...
            $result = executePlainSQL("SELECT * FROM FeatureAnimal1 WHERE animalID = '" . $animalID . "'");

            $columnNames = oci_num_fields($result);
            echo '<table>';
            // Generate table headers with column names
            echo '<tr>';
            for ($i = 1; $i <= $columnNames; $i++) {
                $columnName = oci_field_name($result, $i);
                echo "<th>" . $columnName . "</th>";
            ...
            while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                echo "<tr>";
                for ($i = 0; $i < 7; $i++) {
                    $columnName = oci_field_name($result, $i + 1);
                    echo '<td>';
                    echo '<input type="text" name="' . $columnName . '" value="' . $row[$i] . '">';
                    echo '</td>';
                }
                echo "</tr>";
            }
            echo "</table>";
            //action should be empty if the form submission will be within the same file
            echo '<input type="hidden" id="updateAnimalRequest" name="updateAnimalRequest">';
            echo '<input type="submit" value="Update this Animal" name="AttributeUpdateSubmit"></p>';
            echo "</form>";
        }
        ...
        function handleUpdateAnimalRequest() {
            ...
            //executePlainSQL("UPDATE FeatureAnimal1 SET HabitatID='" . $new_name . "' WHERE AnimalID='" . $old_name . "'");

            executeBoundSQL("UPDATE FeatureAnimal1 SET Age=:bind2, Sex=:bind3, HealthStatus=:bind4,
            Species=:bind5, HabitatID=:bind6, OriginLocation=:bind7 WHERE AnimalID=:bind1", $alltuples);
            OCICommit($db_conn);
        ...
        function handleInsertRequest() {
            ...
            $constat = $_POST['ConservationStatus'];

            if (empty($animalid) || empty($age) || empty($asex) || empty($healthstatus) || empty($species) || empty($habitatid) || empty($originlocation) || empty($constat)) {
            ...
            if(ctype_digit($animalid)) {
                if(ctype_digit($age)) {
                    if ($asex == 'F'|| $asex == 'M') {
                        if ($healthstatus == 'Excellent' || $healthstatus == 'Good' || $healthstatus == 'Fair' || $healthstatus == 'Poor') {
                            $species=str_replace(" ", "", $species);
                            if (ctype_alpha($species)) {
                                if (ctype_digit($habitatid)) {
                                    $originlocation=str_replace(" ", "", $originlocation);
                                    if (ctype_alpha($originlocation)) {
                                        if ($constat == "LC" || $constat == "NT" || $constat == "VE" || $constat == "CE" || $constat == "EW" || $constat == "E" || $constat == "VU" || $constat == "EN") {
                                            ...
                                            $tuple = array (
                                                ...
                                                ":bind6" => $_POST['HabitatID'],
                                                ":bind7" => $_POST['OriginLocation'],
                                                ":bind8" => $_POST['ConservationStatus'],
                                            );

                                            $alltuples = array (
                                                $tuple
                                            );
                                            executeBoundSQL("insert into Animal1 values 
                                            (:bind5, :bind8 )", $alltuples);
                                            executeBoundSQL("insert into FeatureAnimal1 values 
                                            (:bind1, :bind6, :bind5, :bind3, :bind2, :bind4, :bind7 )", $alltuples);
                                            OCICommit($db_conn);
                                        } else {
                                            echo "<script>alert('Conservation Status must be a string that contains no special character (!@#$%^&*, etc.)');</script>";
                                            exit();
                                        }
                                    ...
                                    echo "<script>alert('Habitat ID must be a integer.');</script>";
                                    exit();
                                }
                            } else {
        ...
        function handleHavingNestRequest(){
            global $db_conn;

            $result = executePlainSQL("SELECT fv1.vName, fv1.foodType FROM FoodVendor1 fv1 
        ...
        function    handleGroupRequest(){
            ...
            $result = executePlainSQL("SELECT h2.habitatType, AVG(e.visitorCapacity) as averageCapacity FROM Habitat2 h2, Exhibits e
        ...
        function handleNestGroupRequest(){
            global $db_conn;

            $result = executePlainSQL("SELECT a.species, AVG(a.age) AS averageAge,e.popularityRating
        ...
        function handleDivisionRequest(){
            ...
            $result = executePlainSQL("SELECT species FROM ResidentAnimal1 WHERE NOT EXISTS (SELECT habitatID FROM ResidentAnimal1 a1 WHERE NOT EXISTS 
                                        (SELECT h.habitatID
	...
	// A better coding practice is to have one method that reroutes your requests accordingly. It will make it easier to add/remove functionality.
        function handleGETRequest() {
            if (connectToDB()) {
                ...
                if (array_key_exists('having', $_GET)) {
                    handleHavingNestRequest();
                }
                if (array_key_exists('group', $_GET)) {
                    handleGroupRequest();
                }
                if (array_key_exists('nestgroup', $_GET)) {
                    handleNestGroupRequest();
                }
		...
		if (isset($_POST['reset']) || isset($_POST['updateSubmit']) || isset($_POST['AttributeUpdateSubmit']) || isset($_POST['deleteSubmit']) || isset($_POST['insertSubmit'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['viewAnimalsRequest']) || isset($_GET['joinAnimalsRequest']) 
            || isset($_GET['projectionSubmit']) ||  isset($_GET['selectSubmit'])|| isset($_GET['havingRequest']) || isset($_GET['groupRequest'])
            || isset($_GET['nestgroupRequest'])|| isset($_GET['divisionRequest']) ) {
            handleGETRequest();
        }
		?>
